...
private:
	...
	void handleHttpNetworkConfiguration(AsyncWebServerRequest *request) {
		if (isWebServerRunning()) {
			...
			page->printf_P(HTTP_PASSWORD_FIELD, F("WiFi password:"), "p", "p", "p", 64, (strlen(getPassword()) ?  FORM_PW_NOCHANGE : ""));
	...
	bool loadSettings(){
		...
		settings->setString(PROP_PASSWORD, 64, (settingsOld && settingsOld->existsSetting(PROP_PASSWORD) ? settingsOld->getString(PROP_PASSWORD) : ""));
